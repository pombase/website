...
for my $file_to_scan (@files_to_scan) {
  ...
  while (defined (my $line = <$file>)) {
    ...
    if ($line =~ m!\[.+?\]\(/?([a-zA-Z][^\)\s]*)(:?\s+.*)?\)!) {
      ...
      if ($link =~ m@^metrics/?|^jbrowse2?/?|^gocam/connections$|(spombe/(result|query)|results|gene|gene_protein_features|(pombase_gocam_view|gocam)/(docs|gene)|genotype|term|reference|archive|slim:\w+|vis)/|(query|motif_search|identifier-mapper|internal-details)$@) {
        next;
      }
